-- Booking a bet just saves a selection snapshot behind a voucher code --
-- no money moves, so it should work for guests too (the whole point is
-- sharing a code with a friend who loads it into their own betslip).
-- Placing a real bet still requires auth (enforced in fn_place_bet).
alter table public.booked_bets alter column user_id drop not null;
...
grant execute on function public.fn_book_bet(bet_type, jsonb) to anon;
