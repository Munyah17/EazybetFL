-- Same as the existing fn_request_withdrawal, plus a rate-limit check
-- right after auth: max 5 withdrawal requests per user per hour, to
-- close off balance-probing/abuse via unlimited-frequency requests.
...
begin
  ...
  select public.fn_check_rate_limit('withdrawal:' || v_user_id, 5, 3600) into v_allowed;
